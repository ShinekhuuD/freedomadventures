	...
	<div id="comments" class="post_comments cmsmasters_woo_comments">
		<h2 class="post_comments_title"><?php
			if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' && ( $count = $product->get_review_count() ) )
				printf( esc_html(_n( '%s review for %s', '%s reviews for %s', $count, 'woocommerce' )), $count, get_the_title() );
			else
		...
		<div id="review_form_wrapper">
			<div id="review_form">
				<?php
					...
					$comment_form = array(
						'title_reply'          => have_comments() ? esc_html__( 'Leave a review', 'woocommerce' ) : esc_html__( 'Be the first to review', 'woocommerce' ) . ' &ldquo;' . get_the_title() . '&rdquo;',
					...
					if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' ) {
						$comment_form['comment_field'] = '<p class="comment-form-rating"><label for="rating">' . esc_html__( 'Rate this product?', 'woocommerce' ) .'</label><select name="rating" id="rating">
					...
					$comment_form['comment_field'] .= '<p class="comment-form-comment">' . 
							'<label for="rating">' . esc_html__('Your review:', 'woocommerce') . '</label>' . 
							'<textarea name="comment" id="comment" cols="67" rows="2" placeholder="' . esc_html__('Message', 'woocommerce') . '"></textarea>' . 
						'</p>';
